...
##### Q6: The figure below illustrates how the intersect tool works. Describe in one sentence (in words) how the output relates to the input features. {.unnumbered}
...
**Step 1:** Create a new attribute in **flood_ag_intersect** and name it **SuitableArea**. Use the **Calculate Field** tool to create a new attribute representing NON-flooded area as: Area of the agriculture polygon - flooded area.

Use the fields list and the math symbols to generate the following expression for the **SuitableArea** attribute:
...
**Step 2:** Create another new attribute **ProportionFlooded** and use the **Calculate Field** tool to calculate the proportion of each agricultural polygon that is flooded. Your final values should range between 0 and 1. Make sure the data type for the attribute is **Double** so it can store decimal values.

##### Q11: What expression did you use to calculate ProportionFlooded? (Copy it from the Calculate Field tool). {.unnumbered}

**Step 3:** Next, we will create a **Suitability Score** that combines the **ProportionFlooded** attribute and the **SuitableArea** attribute. First, we will **standardize** the SuitableArea attribute so its values range from 0 to 1 (the same range as the ProportionFlooded.
...
**Step 4:** Create a new attribute to represent a final **Suitability Score** and set the Data Type to **Double**.

Use the equation below to create a Suitability Score based on the proportion of non-flooded land and the total suitable area (standardized). The final values will range from 0 (least suitable) to 2 (most suitable):  

```(1-ProportionFlooded) + SuitArea_Std```
